...
const ActiveOrder = () => {
	...
	const { activeOrder, isActiveOrder, ClearOrder } = useCart();
	const [closing, setClosing] = useState(false);

	if (!activeOrder || pathname === '/orders' || pathname === '/orderStatus')
		return null;
	...
	const showOrder = () => redirect('/orderStatus');

	const createdAt = activeOrder.createdAt;
	const createdToday =
		createdAt && createdAt.toDateString() === new Date().toDateString();
			...
			<div
				...
				<div className='flex flex-col gap-1 overflow-hidden'>
					...
					<div className='text-xs opacity-80 flex gap-2 flex-wrap'>
						{activeOrder.campusName} • {activeOrder.paymentMethod} •{' '}
						{timestampLabel}
					</div>
				</div>
				...
				<div className='ml-auto flex items-center gap-3'>
					...
					<button
						onClick={showOrder}
						className='text-xs font-semibold px-3 py-2 rounded-2xl bg-(--primary-colour)/80 hover:scale-[1.02] active:scale-95 transition-all duration-200 ease-in-out cursor-pointer'>
						Show
					</button>
					<button
						onClick={clearActiveOrder}
						className='text-xs font-semibold px-3 py-2 rounded-2xl bg-(--primary-colour) hover:scale-[1.02] active:scale-95 transition-all duration-200 ease-in-out cursor-pointer'>
						Mark as Delivered
					</button>
				</div>
			</div>
